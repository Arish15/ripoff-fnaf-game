...
// Test 1: Server connectivity and HTML loading
http.get('http://localhost:8000', (res) => {
  let html = '';
  res.on('data', chunk => html += chunk);
  res.on('end', () => {
    console.log('[1] Server Connectivity: ✓ Page loaded');
    
    // Test 2: Check office3d.js is loaded
    const hasOffice3d = html.includes('office3d.js') || fs.existsSync('./src/js/office3d.js');
    if (hasOffice3d) {
    ...
    if (hasThreeJs) {
      console.log('[3] Three.js: ✓ Referenced in HTML');
    } else {
      console.log('[3] Three.js: ⚠ Not explicitly referenced (checking in files)');
    }
    
    // Test 4: Check game.js exists and functions
    ...
    try {
      if (fs.existsSync('./src/js/game.js')) {
        ...
        console.log('[4a] game.js exists: ✓ Found');
        console.log('[4b] startGame function: ' + (hasStartGame ? '✓' : '✗'));
        console.log('[4c] updateHallAnimatronics: ' + (hasUpdateHall ? '✓' : '✗'));
        ...
        console.log('[4] game.js: ✗ Not found');
        issues.push('game.js not found');
        passed = false;
      }
    } catch (e) {
      console.log('[4] game.js check failed: ' + e.message);
      issues.push('Error reading game.js: ' + e.message);
      passed = false;
    }
    
    // Test 5: Check office3d.js for door overlay functions
    ...
    try {
      if (fs.existsSync('./src/js/office3d.js')) {
        ...
        console.log('[5a] office3d.js exists: ✓ Found');
        console.log('[5b] Three.js usage: ' + (hasThree ? '✓' : '✗'));
        console.log('[5c] initDoorOverlay function: ' + (hasInitDoor ? '✓' : '✗'));
        console.log('[5d] drawDoorAnimatronics function: ' + (hasDrawDoor ? '✓' : '✗'));
        console.log('[5e] drawDoorAnim function: ' + (hasDrawAnim ? '✓' : '✗'));
        ...
        console.log('[5] office3d.js: ✗ Not found');
        issues.push('office3d.js not found');
        passed = false;
      }
    } catch (e) {
      console.log('[5] office3d.js check failed: ' + e.message);
      issues.push('Error reading office3d.js: ' + e.message);
      passed = false;
    }
    
    // Test 6: Check for syntax errors in JavaScript files (skip ES modules)
    console.log('\n[6] Syntax Validation:');
  ...
  console.log('❌ FAIL: Server connectivity error - ' + e.message);
  process.exit(1);
});
